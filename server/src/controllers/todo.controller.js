...
const buildQuery = (userId, query) => {
  ...
  if (query.startDate || query.endDate) {
    q.startDate = {};
    if (query.startDate) q.startDate.$gte = new Date(query.startDate);
    if (query.endDate) q.startDate.$lte = new Date(query.endDate);
  }
  if (query.overdue === 'true') {
    q.startDate = q.startDate || {};
    q.startDate.$lt = new Date();
...
const summary = async (req, res) => {
  ...
  const [total, completed, pending, overdue, dueToday] = await Promise.all([
    ...
    Todo.countDocuments({ user: userId, isDeleted: false, startDate: { $lt: now }, status: { $ne: 'completed' } }),
    Todo.countDocuments({ user: userId, isDeleted: false, startDate: { $gte: startOfDay, $lte: endOfDay } })
  ]);
